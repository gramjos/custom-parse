...
def build(vault_path: str, output_dir: str, title: str) -> None:
    ...
    # Write manifest next to the output for reference
    manifest_path = output / DEFAULT_MANIFEST_NAME
    manifest_path.parent.mkdir(parents=True, exist_ok=True)
    manifest_path.write_text(json.dumps(manifest, indent=2) + "\n")
    print(f"  Manifest written to {manifest_path} ({len(manifest['items'])} nodes)")
...
def main():
    ...
    args = ap.parse_args()
    build(args.vault_path, args.output, args.title)

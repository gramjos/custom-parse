...
2.  **Does it need a new AST Node?**
    *   *Does the existing `Node` structure cover it?*
    *   *Answer:* Likely not. Define a new dataclass in `ast_nodes.py` (e.g., `class BlockQuote(Node)`). Don't forget to add it to the `NodeType` enum.
